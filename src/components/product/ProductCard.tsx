// MB Crunchy — Reusable Product Card (Convex-Ready)
...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    addItem({
      ...
      originalPrice: product.discount > 0 && originalPrice > product.price ? originalPrice : product.price,
    });
  ...
  return (
    <Link to={`/product/${product.id}`} onClick={(e) => { if (!product.id) e.preventDefault(); }}>
      <motion.div
        ...
        viewport={{ once: true }}
        transition={{ delay: ((index || 0) % 4) * 0.06, duration: 0.4 }}
      ...
      >
        ...
        <div className="relative aspect-square rounded-t-2xl bg-gradient-to-br from-muted to-muted/50 flex items-center justify-center overflow-hidden">
          {hasImage && !imgError ? (
            ...
            <span className="text-5xl sm:text-6xl select-none">{product.image}</span>
          )}
          ...
          <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 rounded-t-2xl">
            <button onClick={(e) => e.preventDefault()} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-foreground shadow-lg hover:bg-primary hover:text-primary-foreground transition-all duration-200 active:scale-90" aria-label="Quick view">
              <Eye className="h-4 w-4" />
            </button>
            <button onClick={handleToggleWishlist} className={`flex h-9 w-9 items-center justify-center rounded-xl bg-white shadow-lg transition-all duration-200 active:scale-90 ${wishlisted ? "text-red-500" : "text-foreground hover:text-red-500"}`} aria-label={wishlisted ? "Remove from wishlist" : "Add to wishlist"}>
        ...
        <div className="p-3 sm:p-4">
          ...
          {product.tags && product.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-1.5">
              {product.tags.slice(0, 3).map((tag) => (
                <span key={tag} className="rounded-md bg-muted px-1.5 py-0.5 text-[9px] font-medium text-muted-foreground uppercase">{tag}</span>
              ))}
            </div>
          )}
          ...
          {product.shortDescription ? (
            <p className="text-[11px] text-muted-foreground mt-0.5 line-clamp-1">{product.shortDescription}</p>
          ) : product.weight ? (
          ...
          <div className="mt-2.5 flex items-center justify-between">
            ...
            <button onClick={handleAddToCart} disabled={!product.inStock}
              className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-sm shadow-primary/20 transition-all duration-200 hover:bg-primary/90 active:scale-90 disabled:opacity-40 disabled:cursor-not-allowed" aria-label="Add to cart">
